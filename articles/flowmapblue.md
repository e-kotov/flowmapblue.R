# Quick Start with Flowmap.blue R widget

Flowmap.blue widget for R produces an interactive flow map representing
...
For the backgorund maps to work, you also need to set up your Mapbox
access token. You can obtain a free token by signing up at
...
**Note: When using your own data, you need to shape it to the same
structure with column names exactly as in these sample datasets.**
...
You can also visualize flows with date-time information. Here’s an
example with time data included:
...
![](webmedia/demo_datetime.gif)

## Example 3: Flows with Date Only

If your flow data includes only dates (without times), you can also
visualize those:
...
![](webmedia/demo_date.gif)

## Integration with Shiny

If you want to integrate `flowmapblue` into a Shiny application, you can
...
This provides a simple yet powerful way to visualize flows in an
interactive web application.

With these examples, you should be able to quickly get started with
creating and customizing flow maps using the `flowmapblue` package in R.
For more advanced options and features, refer to the package
documentation and explore different settings as needed.
