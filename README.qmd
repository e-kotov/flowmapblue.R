...
You can customize the color of each flow by adding a `color` column to the `flows` data frame. Each color should be a hex color code.

The following example demonstrates how to apply a green color gradient based on the flow magnitude:
...
# Create a color gradient (Light Green to Dark Green)
color_palette <- colorRampPalette(c("#90EE90", "#006400"))
n_colors <- 100
colors <- color_palette(n_colors)

# Map flow counts to colors
# Using rank() to distribute colors evenly across the flows
flow_ranks <- rank(ch_flows$count)
color_indices <- ceiling((flow_ranks / max(flow_ranks)) * n_colors)
ch_flows$color <- colors[color_indices]

flowmap <- flowmapblue(
 ch_locations,
 ch_flows,
 mapboxAccessToken = Sys.getenv('MAPBOX_API_TOKEN'),
 clustering = FALSE, # Disable clustering to see individual flow colors
 darkMode = TRUE,
 animation = FALSE
)
...
You can also automatically color flows based on their magnitude (`count`) using various classification methods and color palettes, similar to `tmap`.

Parameters:
- `palette`: Name of a palette (e.g., "Viridis", "Plasma", "Blues", "Reds") or a vector of colors.
- `style`: Classification method (e.g., "quantile", "jenks", "fisher", "pretty", "equal", "fixed").
- `n`: Number of color intervals.
- `breaks`: Vector of break points (required if `style = "fixed"`).
